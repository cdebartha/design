...
%% Parameters
rho_h = 0.963*(2.205/(3.28084)^3) ; % in slug/ft^3, Altitude = 8000ft
% rho_sl = 1.225*(2.205/(3.28084)^3) ; % in slug/ft^3, Sea-level
V_cruise = 75*3.28084; % in ft/s, Cruise Velocity
...
%% Lift Distribution by Schrenk's Approximation
...
% plotting the drag distribution
figure(2)
plot(x_d_total,D_dist_spw,'k','linewidth',width_val);
plot(x_d_total,D_dist_spw,'k', 'Linewidth', 1.2);
ax = gca;
ax.FontSize = 14;
ax.XAxis.LineWidth = 1.2;
ax.YAxis.LineWidth = 1.2;
...
%% Bending Moment Distribution My due to drag
...
% plotting My distribution due to drag
figure(3)
plot(x_d_total,M_y_due_to_D,'b','linewidth',width_val) ; 
plot(x_d_total,M_y_due_to_D, 'b', 'Linewidth', 1.2) ; 
ax = gca;
ax.FontSize = 14;
ax.XAxis.LineWidth = 1.2;
ax.YAxis.LineWidth = 1.2;
...
%% Bending Moment Distribution Mz due to lift
...
% plotting Mz distribution due to lift
figure(4)
plot(x_d_total,M_z_due_to_L,'b','linewidth',width_val) ; 
plot(x_d_total,M_z_due_to_L, 'b', 'Linewidth', 1.2) ; 
ax = gca;
ax.FontSize = 14;
ax.XAxis.LineWidth = 1.2;
ax.YAxis.LineWidth = 1.2;
...
%% Testing of lift and drag distribution
value_lift =  trapz(x1,lift_shrenk_spw_vari) ;
value_drag = trapz(x_d_total,D_dist_spw) ;

%% Save distributions
save('loads.mat','lift_shrenk_spw_vari','D_dist_spw',...
    'M_y_due_to_D','M_z_due_to_L')